...
- Removed support for old DirectDraw (`ddraw`) output.  
  Use `output = texture`, `texture_renderer = direcx11` instead.
...
- Wayland crashes
- Inconsistent minimum window size with `windowresolution = resizable`
- Window content is not refreshed on Windows and macOS when resizing
- Other bugs
...
This section summarizes development in SVN trunk since DOSBox 0.74 release
(`r3608`) until SVN trunk `r4336` (last revision included in dosbox-staging
0.75.0).  Changes that were backported and released in 0.74-2 or 0.74-3 are
**not** listed in here.
