...
### <a name="summary-new-features" href="#summary-new-features">§</a> Summary of Added Features

- Migrated from SDL1 to SDL2, which:
  - Enabled support for the Mir, Wayland, and Metal display server protocols.
  - Enabled hardware-acceleration for OpenGLES 1 and 2 devices such as Android
    and Raspberry Pi platforms.
- Added support for FLAC, Opus, and MP3 compressed CD-DA tracks.
- Added hardware-accelerated pixel-perfect output.
- Added the ability to resize the window.
- Added the cycle-accurate Nuked OPL3 emulator.
- Added CGA mono-color mode.
...
- Removed support for CD Digital Audio playback from physical disks. It is
...
- Migrated the DOSBox source from SVN to Git, preserving all publicly available
  project history.
...
- Added static analysis checks performed by Clang, Coverity, and PVS-Studio.
...
- The CI system includes gated checks that allow maintainers to focus on code
  ...
  - introduces no added compiler warnings, and
...
### <a name="author-tally" href="#author-tally">§</a> Changes per Author

During the **dosbox-staging 0.75** development period:

- **11 dosbox-staging contributors** submitted **208** pull requests comprised
  of **2,247** commits written by **19 authors**.
- **three DOSBox maintainers** committed **114** changes written by **five** authors.
  These changes have been incorporated into **dosbox-staging**.
...
Mir, Wayland, and OpenGLES 1 and 2; and improves cross-platform consistency.
SDL 2.0 has allowed for major code cleanups (removal of old hacks), enabled
easy support for modern graphic APIs, and opened the field for new features and
improvements.

One example includes a new `output` mode named `texture`, which uses SDL
2D hardware-accelerated drawing API with one of platform-specific backends.
