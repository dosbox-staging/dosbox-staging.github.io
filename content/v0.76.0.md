...
Physical PC Speaker's can sometimes be left in a pushed-out
(or pulled-in) state through constant voltage, called DC-offset,
which on actual hardware isn't a problem because they will remain
mostly silent in that state.

However in dosbox, the emulated PC Speaker's audio samples are mixed
with the other emulated audio devices such as the Sound Blaster or
Adlib.

Eliminating DC-offset is important in dosbox because it can 1) 
cause a pop when (eventually) the next sample played has a value 
"away" from the DC-offset's value 2) harm accurate sound reproduction 
of other sound devices' audio streams by simultaneously pulling (or 
pushing) half the samples either further away from the center line 
(becoming louder) and the other half of the samples closer to the 
center line (becoming quieter).
