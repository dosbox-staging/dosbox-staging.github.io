...
### Added --editconf and --printconf command-line options

Sometimes it's hard to determine the location of `dosbox-staging.conf` file: you
might not know where it's located or it might be hidden away due to certain OS
settings.
...
*Above: the game has left the PC speaker in an offset position, which is detected and silently "wound down" in fraction of a second.*
...
### Skip the splash screen and help banner
