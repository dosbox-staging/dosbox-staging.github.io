...
A blank window may be shown when switching from fullscreen-mode to windowed-mode under the following conditions:
  - using an empty `windowresolution = ` or non-fixed setting value, such as `default` , and
  - using an opengl or surface `output = ` setting.

This can be avoided by:
    - using a fixed `windowresolution` setting, such as`windowresolution = 1280x960`, or
    - using `output = texturepp`, or
    - opening and closing the keyboard mapper (Ctrl+F1 or Cmd+F1 on macOS)
