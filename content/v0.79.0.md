...
### Parallel port DAC separation: Disney Sound Source, Covox Speech Thing, and Stereo-On-1 <a name="LptDac" href="#LptDac">§</a>

Those familiar with DOSBox know that these three devices are emulated with the `disney = true` setting.

Like `sbtype =`, we wanted each LPT DAC type to be configurable and so have deprecated the `disney =` setting in favour of an `lpt_dac =` setting, which takes any of the following:
...
This is paired with a corresponding filter setting, `lpt_dac_filter`, that defaults to approximating the selected devices. Like the other [filter settings](#SoundOutputFilters), it can be toggled on, off, or customized with your own settings. This means that the Disney Sound Source (which included its own little speaker inside it) will sound a lot like you might remember.

Here are examples of the three DAC types with their default filtering (ModPlay Pro & Star Control II's Pkunk MOD):

| **`lpt_dac`** | **Audio Example**                                                                                                                |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------- |
...
Once activated, you can hold `Ctrl` or `Alt` and tap the arrow keys to adjust either the joystick's scalar multiplier or it absolute offset position, respectively.  In essense, you can "counterbalance" the calibration issue.

Each tap of these hotkeys will print out a calibration line in your console log. For example:

```text

2022-09-16 09:26:01.141 | JOYSTICK: y_calibration = 0.576211,0.02
or
2022-09-16 09:26:08.845 | JOYSTICK: y_calibration = 0.5474,-0.005
```

The last-most `y_calibration` or `x_calibration` values can be placed into the affected game's conf setting, also under the `[joystick]` section. 

Here's a video showing how the calibration hotkeys can "pull" the joystick in various directions:

<video controls src="/joystick_calibration_hotkey_demo.webm"> Your browser does not support the <code>video</code> element. </video> 
...
Adds a feature called `automount` that mounts directories-as-drives having having the pattern: drives/[X], where [X] can be letters a through y (z is reserved for DOSBox's drive), when found locally or in the bundled resources, which are part of the release .zip or .dmg package.

This feature is enabled by default, and an bundled `Y:` automount is provided in the release that included a couple FreeDOS 3rd party tools (xcopy.com, deltree.com, debug.com).
...
Each mounted drive can be customized with an optional `[X].conf` file that allows customizing the `type`, `label`, and `path`.  For example, to mount a directory as a CDROM:
...
See the release package's `resources/drives/y` for an example, including its accompanying `y.conf`. Note that currently this feature doesn't support image-file mounts (`.img`, `.bin/cue`, etc).
