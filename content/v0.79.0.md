...
Directories can now be automounted in startup (if the `automount` conf setting is enabled), which will mount directories having having the pattern: `drives/[X]`, where `[X]` is a single letters `a` through `y` representing the DOS drive letter (`z` is reserved for DOSBox's drive).

This feature is enabled by default, and a bundled `Y:` automount is provided in the release reources that included a couple FreeDOS 3rd party tools (xcopy.com, deltree.com, debug.com). Each mounted drive can be customized with an optional `[X].conf` to customize the mount `type`, `label`, and `path`.  For example, to mount a local `drives/d/` directory as a CDROM with the label `SIERRA`, the accompanying `d.conf` would contain:
...
See the release package's `resources/drives/y` and accompanying `y.conf` as an example.
Note: this feature doesn't support image-file mounts (`.img`, `.bin/cue`, etc).
