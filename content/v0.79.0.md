...
* 4. [Graphical Improvements](#GraphicalImprovements)
  * 4.1. [Shader Reload Shorcut](#ShaderReloadShorcut)
  * 4.2. [CGA Palette Override](#CGAPaletteOverride)
  * 4.3. [4.3. Scaled Image Quality Improvements](#ScaledImageQualityImprovements)
...
* 6. [Mount Improvements](#MountImprovements)
  ...
  * 6.3. [Z Drive Customization and Directory Support](#ZDriveCustomizationandDirectorySupport)
  * 6.4. [IMGMOUNT Wildcard Support](#IMGMOUNTWildcardSupport)
  * 6.5. [Automount of Resource Drives](#AutomountofResourceDrives)
* 7. [Localization Improvements](#LocalizationImprovements)
...
* 10. [Developer-related Improvements](#Developer-relatedImprovements)
  * 10.1. [Self-Documenting Bit Twidling with `bit_view`](#Self-DocumentingBitTwidlingwithbit_view)
  ...
  * 10.4. [Continous Integration MSYS2 Workflow Improvements](#ContinousIntegrationMSYS2WorkflowImprovements)
...
2. At the`Z:\>` prompt, run: `config -wc` to write the new configation file. Any existing settings will be retained, however, extraneous comments will be dropped.
...
* For international users, it should detect your language and keyboard layout on startup. We recommend reading the Localization section for details.
...
Like the other audio filters settings, the `lpt_dac_filter` can be toggled on, off, or customized.
...
### 4.1. <a name='ShaderReloadShorcut'></a>Shader Reload Shorcut
...
* Synced CFR, which presents a constant frame rate but is synchronized with the host's refresh rate. This presents only the most recently updated frame when the DOS rate exceeds the host rate.
...
## 7. <a name='LocalizationImprovements'></a>Localization Improvements

### 7.1. <a name='CodePagesandKeyboardLayouts'></a>Code Pages and Keyboard Layouts

The 20 years old FreeDOS code pages (screen fonts) and keyboard layouts were updated to the latest versions, offering countless new definitions for use. See the full list in the following documentation:
...
The soft-modem's reported connect speed can be customized using the serial port's `baudrate:` setting. for example:
...
### 10.1. <a name='Self-DocumentingBitTwidlingwithbit_view'></a>Self-Documenting Bit Twidling with `bit_view`
...
Constructing a `bit_view` is similar to a C bitfield, however unlike C bitfields, `bit_view`s are free from undefined behaviour and have been proven using GCC's and Clang's undefined behaviour sanitizers.
...
### 10.4. <a name='ContinousIntegrationMSYS2WorkflowImprovements'></a>Continous Integration MSYS2 Workflow Improvements
...
The team continues to fix and address static analysis issues as flagged by various analyzers. Here are our Coverity Scan metrics to-date:
...
* Kappa971, for the Italian translation and leading the translation effort
