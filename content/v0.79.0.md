...
### High DPI displays on platforms like macOS<a name="HiDPI" href="#HiDPI">§</a>

A bit of backstory: DOSBox Staging defaults to OpenGL output with sharp bilinear scaling shader. This ensures that no matter the size of the image or pixel dimensions, picture stays crisp and pixels are kept mostly even, with minimal blurring on pixel transitions.

On macOS, due to the way DPI scaling works there, the image was actually drawn at a much lower resolution, usually just a quarter of the window size. Then that image was stretched by the system, which lead to unpleasantly mushy presentation. And we wanted our pixels be so sharp one could cut their eyes.

[picture - TBD]

Now DOSBox Staging is able to handle high DPI displays on macOS fully due to recent serious efforts, be it Retina or not, so you and shaders alike can see all the (physical) pixels. Note that multi-display setups weren't tested as thoroughly --- please report any weird behaviour! High DPI for Linux is currently still uncertain, whereas the issue is less noticeable on Windows since version 0.77.0, but will also be fully supported later, and we may also extend the support to other platforms in the future. Also note that due to the way rendering is currently implemented, you may observe lowered performance on higher resolution displays, especially when running off integrated graphics. This might be remedied in the future as well.
