...
**What you can expect:**
* It should run everything that prior versions did, and generally be plug-and-play.
* The default settings will give you improved audio, refined graphical scaling and color accuracy, more robust mouse handling, and a more usable shell and mount CLI.
* There are many new and some deprecated conf settings: your console log will inform you with orange-colored warning messages to help you.
* It typically runs as fast or a bit faster than prior versions, especially for those late 90's protected mode and software-rendered 3D games.
...
Like all the audio filters, the `lpt_dac_filter` can be toggled on, off, or customized with your own settings.
...
Although this doesn't actually change the underling baud rate, it can help satify old modem software if it's unable to handle (or parse) the larger connect values.

###  9.4. <a name='HandlingOptionsforMemoryFaultsinBuggyGams'></a>Handling Options for Memory Faults in Buggy Gams

Extremely rare programming bugs :smirk: in some DOS games (and programs) may cause them to accidentally write data beyond their allocated memory region, similar to "painting outside the lines". DOSBox is great at catching these instances: it flags the corruption and quits, preventing the program from barreling on and possibling doing more harm such as writing corrupt savegames, documents, and so on.

DOSBox Staging defaults to this same behavior, but adds the `[dosbox] mcb_fault_strategy` conf setting:
