...
Add country support for different date, time, and decimal formats #1483

Adds support for COUNTRY which allows DOS software to read system locale and use locale specific date, time and decimal formats (Wengier).

Allow read-only mount of disk images (#1499) #1509

Adds support for read-only disk images. Read-only flag of disk images is honored or you can specify IMGMOUNT with -ro for read-only access (Wengier).

Support for piping from the command-line #1517

Adds support for Unix style pipes for command line like DIR | MORE. Also adds support for multiple pipes like DIR | SORT | MORE (Wengier).

Implement file attribute support and ATTRIB command #1534

Adds support for file attributes through ATTRIB. Archive, read-only, system and hidden file attributes are now supported with the exception of system and hidden attributes on mounted local drives for non-Windows platforms (Wengier).

Add wildcard support for IMGMOUNT command #1584

IMGMOUNT now supports wildcards for mounting which allows you to mount a set of floppy disk images just by specifying IMGMOUNT a d:\*.img -t floppy, for example (Wengier).
