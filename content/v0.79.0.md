...
As an example, a bundled `Y:` automount is provided in the release's `reources/` directory that bundles a couple FreeDOS 3rd party tools (xcopy.com, deltree.com, debug.com) along with an accompanying `y.conf` to add it to the PATH.

Note that this feature doesn't yet support image-file mounts, such as `.img`, `.bin/cue`, or `.iso`.
