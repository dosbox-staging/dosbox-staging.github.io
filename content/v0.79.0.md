...
Directories can now be automounted (if the `automount` conf setting is enabled).

Automount directories are single-letter directories: `a` through `y`,  representing the DOS drive letter where `z` is reserved for DOSBox's drive) placed inside a local `drives/`, directory, relative to the startup local or provided in a scanned resource location (like your dosbox conf directory or the release package's resource area).

Each drive can be customized with an optional `<letter>.conf` to customize its mount `type`, `label`, and `path`.  For example, to mount a local `drives/d/` directory as a CDROM with the label `SIERRA`, the accompanying `d.conf` would contain:
...
A bundled `Y:` automount is provided in the release reources that includes a couple FreeDOS 3rd party tools (xcopy.com, deltree.com, debug.com). You can see this in the release package's `resources/drives/y` directory and accompanying `y.conf`, as an example.

Note that this feature doesn't support image-file mounts (`.img`, `.bin/cue`, etc).
...
- DOS's emulated main memory (up to 64 MiB) and the video card's memory (to up 8 MiB) are both now allocated statically to reduce pressure on heap storage. [Ref: https://github.com/dosbox-staging/dosbox-staging/pull/1714]
