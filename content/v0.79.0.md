...
Similar to `sbtype =`, we wanted each LPT DAC type to be configurable and so have deprecated the `disney =` setting in favour of an `lpt_dac =` setting, which takes any of the following:
...
This is paired with a corresponding filter setting, `lpt_dac_filter`, that defaults to approximating the selected devices. Like the other filter settings, it can be toggled on, off, or customized with your own settings. This means that the Disney Sound Source (which included its own little speaker inside it) will sounds a lot like you might remember.

Here are some examples of the three (ModPlay Pro & Start Control Pkunk .MOD):
