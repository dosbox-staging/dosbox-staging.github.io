...
### High DPI displays on macOS<a name="HiDPI" href="#HiDPI">§</a>
...
Now DOSBox Staging is able to handle high DPI displays on macOS due to recent efforts, be it Retina or not, so you and shaders alike can see all the (physical) pixels. Note that multi-display setups weren't tested as thoroughly --- please report any weird behaviour! The high DPI issue is less noticeable on Windows since version 0.77.0, but will also be fully supported later, whereas high DPI for Linux is still uncertain. Also note that due to the way rendering is currently implemented, you may observe lowered performance on higher resolution displays, especially when running off integrated graphics. This might be remedied in the future as well.
